...
func toExternalProvider(p cp.Provider) (*ExternalProvider, error) {
	...
	case *provider.ProxySetProvider:
		...
		// v1.19.27: SubscriptionInfo is private; extract via JSON marshal/unmarshal
		var si *provider.SubscriptionInfo
		if data, err := json.Marshal(psp); err == nil {
			var apiProvider struct {
				SubscriptionInfo *provider.SubscriptionInfo `json:"subscriptionInfo"`
			}
			if json.Unmarshal(data, &apiProvider) == nil {
				si = apiProvider.SubscriptionInfo
			}
		}
